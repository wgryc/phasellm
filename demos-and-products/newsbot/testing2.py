"""
In this file, we compare Claude with two different prompts. One is from testing.py and the second is a new one, focused on storytelling.
...
def getArticlesAndSummarize2(llm, query, news_articles):
    ...
    system = f"You are an AI journalist researching {query}. You will be provided a list of headlines and additional information about articles (e.g., URL and maybe description of the article). Please summarize the stories into a short paragraph with a focus on being pithy, informative, and addressing what you think are the reader's top priorities. Please ensure you cite your sources by providing footnotes. For example, if you include a summary from an article with $URL, include the $URL as a footnote. The footnote should use square brackets in the actual text, [#], with the numbers increasing in order from 1 to $N, where $N is the number of articles you're citing. If you do not reference a specific article from the list you are provided, you need not cite it.\n\nPlease follow these additional requirements:\n1. Do not include a title or description. Simply provide the paragraph and footnotes.\n2. Footnotes should be numbered consecutively -- 1, 2, 3, and so on.\n3. Avoid opinions or commentary unless you think there is a higher-level summary that is worth providing."
...
llm_1 = ClaudeWrapper(anthropic_api_key) #OpenAIGPTWrapper(openai_api_key, model="gpt-4")
